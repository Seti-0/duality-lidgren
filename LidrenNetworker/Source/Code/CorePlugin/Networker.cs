...
{
    public class LidgrenNetworker : INetworker
    {
        ...
        private void EnsureNetPeerShutdown<T>(ref T peer) where T : NetPeer
        {
            ...
            switch(peer.Status)
            {
                ...
                case NetPeerStatus.Running:
                    ...
                    Logs.Game.WriteError($"{name} running when {nameof(LidgrenNetworker)} was disposed.");
                    break;

                case NetPeerStatus.ShutdownRequested:
                    Logs.Game.WriteWarning($"{name} still shutting down when {nameof(LidgrenNetworker)} was disposed.");
                    break;
            }
